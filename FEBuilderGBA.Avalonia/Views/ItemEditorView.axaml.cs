...
namespace FEBuilderGBA.Avalonia.Views
{
    public partial class ItemEditorView : TranslatedUserControl, IEmbeddableEditor, IPickableEditor, IDataVerifiableView
    {
        ...
        List<(uint id, string name)> _additionalDamageTypeList = new();
        List<(uint id, string name)> _whenUsedList = new();
        ...
        uint GetSelectedWeaponTypeId()
        {
            ...
            return selectedIndex >= 0 &&
               selectedIndex < _weaponTypeList.Count
        ...
        {
            _weaponTypeList =
                ComboResourceHelper.MakeWeaponTypeList(selectedId);

            WeaponTypeCombo.ItemsSource =
                _weaponTypeList.Select(x => x.name).ToList();

            WeaponTypeCombo.SelectedIndex =
                selectedId.HasValue
